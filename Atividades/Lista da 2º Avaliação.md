...
\end{align*}$$
  Seja $\Gamma_{i}(\Sigma) = \{A \in \Sigma \mid |A| \le i \}$. Um conjunto que contém as strings $A$ de tamanho menor ou igual a $i$. Há pelo menos um elemento $x \in \Sigma$ tal que $|f(x)| \lt |x|$, $\forall f$. Seja $k$ esse elemento. Logo $f(k)$ pertenceria à $B = \Gamma_{|k|-1}(\Sigma)$, Mas se tentarmos encaixar $f(k)$ em $B$, pelo princípio da casa dos pombos, vai existir $z \in \Sigma \mid |z| = |k| - 1 \land |f(z)| \le z$, com $z \ne k$  e $f(z)$ = $f(k)$ que também pertence à $B$. O que faz com que $f^{-1}$ não exista portanto esse algoritmo é impossível. 