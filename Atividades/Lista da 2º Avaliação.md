...
\end{align*}$$
  Seja $\Gamma_{i}(\Sigma) = \{A \subseteq \Sigma \mid |A| = i \}$. Um conjunto que contém os subconjuntos que contém as strings $A$ de tamanho $i$. Há pelo menos um elemento $x \in \Sigma$ tal que $|f(x)| \lt |x|$, $\forall f$. Seja $k$ esse elemento. Logo $f(k)$ pertence à um dos elementos da sequência $B = \Gamma_a(\Sigma), a=1,2,...,|k|- 1$.  Seja $\delta \mid \forall c \in \delta, |c| = s$ esse subconjunto, note que há um elemento $z \in \Sigma \mid |z| = |k| - 1 \land |f(z)| \le z$  suponha que $z \in \delta$. Logo $f(z)=f(k)$, pois $\delta$ contém todos as bit strings de tamanho $s$. Logo há um elemento $x \in \Sigma$ que mapeia para dois elementos diferentes. Portanto $f$ não é injetiva, logo $f^{-1}$ não existe portanto o algoritmo é impossível.