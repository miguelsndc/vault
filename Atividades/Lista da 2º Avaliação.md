...
\end{align*}$$
  Seja $\Gamma_{i}(\Sigma) = \{A \in \Sigma \mid |A| \le i \}$, com $i \ge 0$,  um função que define um subconjunto de $\Sigma$ que contém as strings de comprimento menor ou igual a $i$. Há pelo menos um elemento $x \in \Sigma$ tal que $|f(x)| \lt |x|$, $\forall f$. Seja $k$ esse elemento. Logo $f(k)$ pertence à $B = \Gamma_{|k|-1}(\Sigma)$, Suponha que $z \in \Sigma \mid |z| = |k| - 1 \land |f(z)| \le |z|$.
  Caso $f$ não seja sobrejetiva, não existe $f^{-1}$.
  Caso $f$