...
\end{align*}$$
  Seja $\Gamma_{i}(\Sigma) = \{A \in \Sigma \mid |A| \le i \}$, com $i \ge 0$,  um função que define um subconjunto de $\Sigma$ que contém as strings $A$ de tamanho menor ou igual a $i$. Há pelo menos um elemento $x \in \Sigma$ tal que $|f(x)| \lt |x|$, $\forall f$. Seja $k$ esse elemento. Logo $f(k)$ pertence à $B = \Gamma_{|k|-1}(\Sigma)$, Porém ao encaixar $f(k)$ em $B$, pelo princípio da casa dos pombos, vai existir um$z \in \Sigma \mid |z| = |k| - 1 \land |f(z)| \le |z|$, com $z \ne k$  e $f(z)$ = $f(k)$, que também pertence à $B$. Logo $f^{-1}$ não existe.